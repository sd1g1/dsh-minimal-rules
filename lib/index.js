...
//   - 规则组装成一条独立的 user 消息（<system-reminder> 包裹），插在
...
const INSTRUCTION_INTRO = [
  "以下是我们的协作约定和提示信息",
  "更具体、更靠近当前目录的内容优先；除非我在本轮给出明确冲突的提示，否则以约定内容为准。",
].join(" ");
...
/** 把一段规则原文组装成带来源路径的 instruction section。 */
...
/**
 * 把各 rule section 组装成一条独立 user 消息的文本。
 * 使用官方 dsh-agent-instructions 同款 <system-reminder> 语义，
 * 并转义正文中可能出现的闭合标签，避免提前终止提醒区域。
 */
export function buildInstructionReminder(sections) {
